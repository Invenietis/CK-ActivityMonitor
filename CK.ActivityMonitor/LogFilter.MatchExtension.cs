...
namespace CK.Core
{
    ...
    {
        /// <summary>
        /// Matches a <see cref="LogClamper"/>: it can be a predefined filter as ("Undefined", "Debug", "Verbose", etc.)  
        ...
        public static bool MatchLogClamper( this StringMatcher m, out LogClamper c )
        {
            c = LogClamper.Undefined;
            if( !m.MatchLogFilter( out var f ) ) return false;
            c = new LogClamper( f, m.TryMatchChar( '!' ) );
            return true;
        }
