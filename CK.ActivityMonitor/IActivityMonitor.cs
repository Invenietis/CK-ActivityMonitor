...
namespace CK.Core
{
    ...
    public interface IActivityMonitor : IActivityLogger
    {
        ...
        /// <summary>
        /// Gets a thread safe <see cref="IActivityLogger"/> for this monitor.
        /// This is not null if and only if the monitor has been initialized with a <see cref="DateTimeStampProvider"/>.
        /// Log lines generated by this thread safe logger are emitted through the StaticLogger, not through
        /// the <see cref="IActivityMonitor.Output"/>: the logs are then sequentially sent BUT cannot be observed by
        /// the monitor's clients.
        /// </summary>
        ActivityMonitor.ThreadSafeLogger? Logger { get; }
    }
